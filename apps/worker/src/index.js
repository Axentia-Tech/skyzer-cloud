...
const createServerWorker = new Worker('provisioning', async (job) => {
    if (job.name === 'create_server') {
        ...
        try {
            ...
            const pteroResponse = await pteroAxios.post('/api/application/servers', {
                ...
                limits: {
                    memory: order.product.limits.ram,
                    swap: 0,
                    disk: order.product.limits.disk,
                    io: 500,
                    cpu: order.product.limits.cpu,
                },
                feature_limits: {
                    backups: order.product.limits.backups,
                    databases: order.product.limits.databases,
                    allocations: 1,
                },
                allocation: {
            ...
            const pteroServerId = pteroResponse.data?.object?.id;
            // Update server in DB
            ...
            logger.info(`Server created successfully: ${pteroServerId} for order ${orderId}`);
            return {
                success: true,
                pteroServerId,
            };
            ...
            // Mark as failed
            const { orderId, serverId } = job.data;
            await prisma.server.update({
                where: { id: serverId },
                data: { status: 'failed' },
            });
...
const deleteServerWorker = new Worker('provisioning', async (job) => {
    if (job.name === 'delete_server') {
        logger.info(`Processing delete_server job: ${job.id}`);
        try {
            const { serverId, pteroServerId } = job.data;
